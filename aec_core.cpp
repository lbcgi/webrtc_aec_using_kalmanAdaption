...
static int SignalBasedDelayCorrection(AecCore* self) {
  ...
  if ((last_delay >= 0) && (last_delay != self->previous_delay) &&
    ...
    if (do_correction == 1) {
      ...
      delay_correction += delay > self->shift_offset ? self->shift_offset : 1;//delay上限为shift_offset， 下限为1
      self->shift_offset--;
      self->shift_offset = (self->shift_offset <= 1 ? 1 : self->shift_offset);//shift_offset无上限，下限为1
...
static void ProcessNearendBlock(
  ...
  // Block wise delay estimation used for logging 对用于延迟估计的中间变量进行维护
...
#else
  aec->delay_agnostic_enabled = 1;//debug
